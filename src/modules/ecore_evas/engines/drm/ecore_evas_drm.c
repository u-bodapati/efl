...
_ecore_evas_drm_init(Ecore_Evas_Engine_Drm_Data *edata, const char *device)
{
   ...
   edata->launcher = ecore_drm2_launcher_connect("seat0", 0, EINA_TRUE);
   if (!edata->launcher)
     {
        ERR("Failed to create launcher");
        goto launcher_err;
     }

   edata->device = ecore_drm2_device_find("seat0");
   if (edata->device)
     {
        edata->fd =
          ecore_drm2_launcher_open(edata->launcher, edata->device, -1);
        if (edata->fd < 0)
          {
             ERR("Failed to open device %s", edata->device);
             goto open_err;
          }
     }
   else
     {
        ERR("Could not find drm device");
        goto dev_err;
     }
   ...
   edata->output = ecore_drm2_output_find(edata->launcher, edata->x, edata->y);
   if (!edata->output)
...
dev_err:
   ecore_drm2_launcher_disconnect(edata->launcher);
launcher_err:
   ecore_drm2_shutdown();
init_err:
   return --_drm_init_count;
...
_ecore_evas_drm_shutdown(Ecore_Evas_Engine_Drm_Data *edata)
{
   ...
   ecore_drm2_planes_destroy(edata->launcher, edata->fd);
   ecore_drm2_outputs_destroy(edata->launcher, edata->fd);
   ecore_drm2_input_shutdown(edata->launcher);
   ecore_drm2_launcher_close(edata->launcher, edata->fd);
   eina_stringshare_del(edata->device);
   ...
   if (obj != old)
     {
        ...
        evas_object_pass_events_set(ee->prop.cursor.object, EINA_TRUE);
...
_drm_fullscreen_set(Ecore_Evas *ee, Eina_Bool on)
{
   ...
   /* TODO: finish. Need API in ecore_drm2 to get output geom */
   /* if (on) */
   /*   { */
   /*   } */
   /* else */
