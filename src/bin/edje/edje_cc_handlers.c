...
Eina_Bool current_group_import = EINA_FALSE;
...
ob_collections_group(void)
{
   ...
   current_group_import = EINA_FALSE;
        ...
        if (edje_file_import)
          {
             ...
             current_group_import = EINA_TRUE;
          }
...
#define STRDUP(x) x ? strdup(x) : NULL
   ...
   if ((ed->color_class) && (current_group_import))
     {
        Edje_Color_Class *cc;
        Edje_Color_Class_Parser *ccp;
        Eina_List *l;
        ...
        EINA_LIST_FOREACH(edje_file->color_classes, l, cc)
          {
             if (!strcmp(ed->color_class, cc->name))
               {
                  overriden = EINA_TRUE;
                  break;
               }
