   ...
   EINA_LIST_FOREACH(dev_list, l, edev)
     {
        ...
        if ((evas_device_class_get(edev) == (Evas_Device_Class)clas) &&
            (eina_streq(evas_device_description_get(edev), name)))
          return edev;
     }
   return NULL;
}
   ...
   EINA_LIST_FOREACH(dev_list, l, edev)
     {
        if (!edev) continue;
        if ((evas_device_class_get(edev) == (Evas_Device_Class)clas) &&
            (eina_streq(evas_device_description_get(edev), identifier)))
          return;
     }
...
static void
_ecore_event_evas_del_evas_device(Evas *e, const char *name EINA_UNUSED,
                                  const char *identifier, Ecore_Device_Class clas)
{
   const Eina_List *dev_list;
   const Eina_List *l;
   Evas_Device *edev;
...
{
   Ecore_Event_Device_Info *e = event;
   Ecore_Input_Window *lookup;
