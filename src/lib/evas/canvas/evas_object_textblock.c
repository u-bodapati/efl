...
struct _Ctxt
{
   ...
   Evas_Textblock_Extension_Data *oext;
...
_layout_find_paragraph_by_y(Evas_Object_Textblock_Paragraph *start,
      Evas_Object_Textblock_Paragraph **par_index, Evas_Coord y)
{
   Evas_Object_Textblock_Paragraph *par;
   int i;
   ...
   for (i = 0 ; i < TEXTBLOCK_PAR_INDEX_SIZE ; i++)
     {
        if (!par_index[i] || (par_index[i]->y > y))
          {
             break;
          }
        start = par_index[i];
 ...
 */
static void
_paragraph_clear(Evas_Object_Textblock_Paragraph *par)
{
   while (par->lines)
     {
        Evas_Object_Textblock_Line *ln;
 ...
 */
static void
_paragraphs_clear(Evas_Object_Textblock_Paragraph *pars)
{
   Evas_Object_Textblock_Paragraph *par;
...
loop_advance:
   ...
   printf("-- Finalize: c->y = %d, c->par->y = %d\n", c->y, c->par->y);
   c->ln->y = c->y - c->par->y;
   c->ln->h = c->ascent + c->descent;
   printf("----- Line set to: y=%d, h=%d\n", c->ln->y, c->ln->h);
...
static inline void
_layout_paragraph_render(Evas_Object_Textblock_Paragraph *par)
{
   if (par->rendered)
      return;
...
#define _INLIST_OP(x, op) \
   ((__typeof__(x))(EINA_INLIST_GET(x)-> op))
#define _INLIST_FUNC(x, func, y) \
   ((__typeof__(x))(eina_inlist_ ##func (EINA_INLIST_GET(x), EINA_INLIST_GET(y))))
#define _INLIST_FUNC_VAR(x, func, y, z) \
   ((__typeof__(x))(eina_inlist_ ##func \
      (EINA_INLIST_GET(x), EINA_INLIST_GET(y), \
       EINA_INLIST_GET(z))))

/* This merges par1 and par2 */
static Evas_Object_Textblock_Paragraph *
_layout_paragraph_merge(
      Evas_Object_Textblock_Paragraph *par1,
      Evas_Object_Textblock_Paragraph *par2)
{

   par1->logical_items = eina_list_merge(par1->logical_items, par2->logical_items);
   /* there is no EINA_INLIST_MERGE :( */
   while (par2)
     {
        Evas_Object_Textblock_Paragraph *p = par2;
        par2 = _INLIST_FUNC(par2, remove, par2);
        par1 = _INLIST_FUNC(par1, append, p);
     }
   return par1;
}
/* Yes, paragraphs have a "split" relation, for handling paragraphs across
 * different textblock objects i.e. Textblock Extension */
/* This takes c->par and splits it */
static Evas_Object_Textblock_Paragraph *
_layout_paragraph_split(Ctxt *c, Eina_List *l)
{
   /* new_par is also the head of the new INLIST of paragraphs in oext */
   Evas_Object_Textblock_Paragraph *new_par;
   Evas_Object_Textblock_Paragraph *nextp;

   new_par = calloc(1, sizeof(Evas_Object_Textblock_Paragraph));
   c->par->logical_items = eina_list_split_list(
         c->par->logical_items, l, &new_par->logical_items);

   new_par->text_node = c->par->text_node;
   new_par->is_bidi = c->par->is_bidi;
   new_par->direction = c->par->direction;

   /* there is no EINA_INLIST_SPLIT :( */
   while ((nextp = _INLIST_OP(c->par, next)))
     {
        c->paragraphs = _INLIST_FUNC(c->paragraphs, remove, nextp);
        new_par = _INLIST_FUNC(new_par, append, nextp);
     }
   return new_par;
}

static Evas_Object_Textblock_Item *
_layout_last_item_get(Ctxt *c)
{
   Evas_Object_Textblock_Line *last_ln;
   Evas_Object_Textblock_Item *last_it = NULL;
   if (!c->par->lines) return NULL;

   last_ln = (Evas_Object_Textblock_Line *) EINA_INLIST_GET(c->par->lines)->last;
   if (last_ln->items)
     {
        last_it = (Evas_Object_Textblock_Item *) EINA_INLIST_GET(last_ln->items)->last;
     }
   return last_it;
}
...
{
   Evas_Object_Protected_Data *obj = eo_data_scope_get(eo_obj, EVAS_OBJECT_CLASS);
   Evas_Textblock_Extension_Data *o = eo_data_scope_get(eo_obj, EVAS_TEXTBLOCK_EXTENSION_CLASS);
   Evas_Object_Textblock_Item *last_it;
...
static void
_evas_textblock_extension_changed(Evas_Textblock_Extension_Data *o, Evas_Object *eo_obj)
{
   Evas_Object_Protected_Data *obj = eo_data_scope_get(eo_obj, EVAS_OBJECT_CLASS);
...
_find_layout_item_line_match(Evas_Object *eo_obj, Evas_Object_Textblock_Node_Text *n, size_t pos, Evas_Object_Textblock_Line **lnr, Evas_Object_Textblock_Item **itr)
{
   ...
   if (found_par)
     {
        _layout_paragraph_render(found_par);
...
evas_textblock_cursor_char_coord_set(Evas_Textblock_Cursor *cur, Evas_Coord x, Evas_Coord y)
{
   ...
   found_par = _layout_find_paragraph_by_y(o->paragraphs, o->par_index, y);
   if (found_par)
     {
        _layout_paragraph_render(found_par);
        EINA_INLIST_FOREACH(found_par->lines, ln)
...
_textblock_extension_find(Extension_Info *infos, Evas_Object *ext_eo)
{
   Extension_Info *info;

   EINA_INLIST_FOREACH(EINA_INLIST_GET(infos), info)
     {
        if (info->ext_eo == ext_eo) return info;
     }
...
static Extension_Info *
_textblock_extension_add(Extension_Info *infos, Evas_Object *ext_eo)
{
   Extension_Info *info = calloc(1, sizeof(Extension_Info));
   info->ext_eo = ext_eo;
   infos = (Extension_Info *) eina_inlist_append(
         EINA_INLIST_GET(infos), EINA_INLIST_GET(info));
...
{
   Evas_Textblock_Extension_Data *oext =
      eo_data_scope_get(ext_eo, EVAS_TEXTBLOCK_EXTENSION_CLASS);
   _textblock_extension_add(o->extensions, ext_eo);
   _evas_textblock_changed(o, eo_obj);
   _evas_textblock_extension_changed(oext, ext_eo);
   /* TODO: add callback for deletion handling,
      ...
      Evas_Object *ext_eo)
{
   ...
   Extension_Info *info = _textblock_extension_find(o->extensions, ext_eo);
...
static void
_do_render(Evas_Object_Protected_Data *obj,
           Evas_Object_Textblock_Paragraph *paragraphs,
           Evas_Object_Textblock_Paragraph **par_index,
           void *output, void *context, void *surface,
           int x, int y, Eina_Bool do_async)
{
...
#define DRAW_FORMAT(oname, oy, oh) \
   ...
   while (0)

     {
        ...
        if (look_for_y >= 0)
           start = _layout_find_paragraph_by_y(paragraphs, par_index, look_for_y);
...
EAPI void
ppar_lines(Evas_Object_Textblock_Paragraph *par)
{
   printf("*******************:\n");
   printf("Resultant Paragraph:\n");
   Evas_Object_Textblock_Line *ln;
   Evas_Object_Textblock_Item *it;
   EINA_INLIST_FOREACH(EINA_INLIST_GET(par->lines), ln)
     {
        printf(" -- Line (rel_y=%d): y=%d\n", ln->y, par->y + ln->y);
        EINA_INLIST_FOREACH(EINA_INLIST_GET(ln->items), it)
          {
             pitem(it);
          }
     }
