                           ...
                           const Eo *from, const Eo *to, double pos_map)
{
   ...
   from_pd = eo_data_scope_get(from, EFL_GFX_SHAPE_CLASS);
   to_pd = eo_data_scope_get(to, EFL_GFX_SHAPE_CLASS);
                              ...
                              const Eo *with)
{
   ...
   with_pd = eo_data_scope_get(with, EFL_GFX_SHAPE_CLASS);
...
_efl_gfx_shape_dup(Eo *obj, Efl_Gfx_Shape_Data *pd, Eo *dup_from)
{
   ...
   from = eo_data_scope_get(dup_from, EFL_GFX_SHAPE_CLASS);
   if (!from) return ;
                             ...
                             double x, double y, double radius)
{
   ...
   _efl_gfx_shape_append_arc_to(obj, pd, x - radius, y, radius, radius, 0,
                          ...
                          double start_angle,double sweep_length)
{
   ...
   if (fabs(sweep_length - 360.0) < 0.005)
     {
        // draw a ellipse
        _efl_gfx_shape_append_move_to(obj, pd, x+w, cy);
        // draw two 180 degree arcs.
        _efl_gfx_shape_append_arc_to(obj, pd, x, cy, w/2, h/2, 0, EINA_FALSE,
                                      EINA_FALSE);
        _efl_gfx_shape_append_arc_to(obj, pd, x+w, cy, w/2, h/2, 0, EINA_FALSE,
                                     EINA_FALSE);
        return;
     }
   ...
   if (_efl_gfx_is_new_path(pd->commands))
      _efl_gfx_shape_append_move_to(obj, pd, sx, sy);
   else
      _efl_gfx_shape_append_line_to(obj, pd, sx, sy);
   ...
   _efl_gfx_shape_append_arc_to(obj, pd, ex, ey, w/2, h/2, 0, large_arc,
                                sweep_flag);
}

void
_efl_gfx_shape_append_rounded_rect(Eo *obj, Efl_Gfx_Shape_Data *pd,
                                        double x, double y, double w, double h,
                                        double xr,double yr)
{
   if (xr > 100)
   ...
   _efl_gfx_shape_append_arc(obj, pd, x, y, rxx2, ryy2, 180, -90);
   _efl_gfx_shape_append_arc(obj, pd, x + w -rxx2, y, rxx2, ryy2, 90, -90);
   _efl_gfx_shape_append_arc(obj, pd, x + w -rxx2, y + h - ryy2, rxx2, ryy2, 0,
                           -90);
   _efl_gfx_shape_append_arc(obj, pd, x, y + h - ryy2, rxx2, ryy2, 270,
                            -90);
   _efl_gfx_shape_append_close(obj, pd);
}
